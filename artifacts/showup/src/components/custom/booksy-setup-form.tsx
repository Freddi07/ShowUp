...
import type { BooksyConnectResponse, BooksyStatusResponse } from '@/lib/contracts/booksy';
...
export function BooksySetupForm() {
  const [apiKey, setApiKey] = useState('');
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | undefined>();
  const [success, setSuccess] = useState(false);
  const [status, setStatus] = useState<BooksyStatusResponse | null>(null);
  ...
  async function handleConnect(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiFetch<BooksyConnectResponse>('/api/integrations/booksy/connect', {
        method: 'POST',
        body: JSON.stringify({ apiKey }),
      });
      setSuccess(true);
      const newStatus = await apiFetch<BooksyStatusResponse>('/api/integrations/booksy/status', {
        method: 'GET',
      });
      setStatus(newStatus);
    } catch (err) {
      ...
      setError('Kunne ikke koble til Booksy. Sjekk at API-nøkkelen er gyldig.');
    }
  ...
  useState(() => {
    apiFetch<BooksyStatusResponse>('/api/integrations/booksy/status', { method: 'GET' })
      .then(setStatus)
      .catch(() => {});
  });
    ...
    <div className="flex flex-col gap-4">
      ...
      <ol className="space-y-2 text-sm text-muted-foreground">
        <li className="flex gap-2">
          ...
          <span>Logg inn på Booksy Partners-portalen</span>
        </li>
        <li className="flex gap-2">
          <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">
            2
          </span>
          <span>Gå til API-innstillinger og generer en partner-API-nøkkel</span>
        </li>
        <li className="flex gap-2">
          <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">
            3
      ...
      <form onSubmit={handleConnect} className="flex flex-col gap-3" noValidate>
        <div className="space-y-1">
          <Label htmlFor="booksy-api-key">API-nøkkel</Label>
          <Input
            id="booksy-api-key"
            type="password"
            value={apiKey}
            onChange={(e) => setApiKey(e.target.value)}
            placeholder="Din Booksy partner API-nøkkel"
            autoComplete="off"
          />
        </div>
        ...
        {success && (
          <Alert className="border-green-600 bg-green-50 text-green-800">
            ...
            <AlertDescription>Koblet til Booksy!</AlertDescription>
          </Alert>
        )}
