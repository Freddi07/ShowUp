...
import { EmekanikerUploadResponseSchema } from '@/lib/contracts/emekaniker';
...
export async function POST(req: Request) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const externalCustomerId = `emekaniker_${name}_${dateStr}`;
    ...
    const customer = await prisma.customer.findFirst({
      where: { userId: user.id, externalId: externalCustomerId, source: 'emekaniker' },
    });

    const apptExternalId = `emekaniker_${name}_${scheduledAt.toISOString()}`;
  ...
  return NextResponse.json(
    EmekanikerUploadResponseSchema.parse({
      imported,
      skipped,
