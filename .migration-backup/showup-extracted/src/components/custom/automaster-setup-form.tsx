...
import type {
  AutomasterStatusResponse,
  AutomasterUploadResponse,
} from '@/lib/contracts/automaster';
...
export function AutomasterSetupForm() {
  const [file, setFile] = useState<File | null>(null);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | undefined>();
  const [success, setSuccess] = useState(false);
  const [result, setResult] = useState<AutomasterUploadResponse | null>(null);
  const [status, setStatus] = useState<AutomasterStatusResponse | null>(null);
  ...
  async function handleUpload(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = await apiFetch<AutomasterUploadResponse>('/api/integrations/automaster/upload', {
        method: 'POST',
        body: formData,
      });
      setResult(data);
      setSuccess(true);
      const newStatus = await apiFetch<AutomasterStatusResponse>(
        '/api/integrations/automaster/status',
        { method: 'GET' },
      );
      setStatus(newStatus);
    } catch (err) {
  ...
  useState(() => {
    apiFetch<AutomasterStatusResponse>('/api/integrations/automaster/status', { method: 'GET' })
      .then(setStatus)
      .catch(() => {});
  });
    ...
    <div className="flex flex-col gap-4">
      ...
      <ol className="space-y-2 text-sm text-muted-foreground">
        <li className="flex gap-2">
          ...
          <span>Åpne Automaster og gå til Rapporter → Kunder</span>
        </li>
        <li className="flex gap-2">
          <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">
            2
        ...
        <li className="flex gap-2">
          ...
          <span>Last opp filen her for å importere avtalene</span>
        </li>
      </ol>
