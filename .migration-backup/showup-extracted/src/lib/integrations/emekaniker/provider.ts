...
export class EmekanikerProvider implements IntegrationProvider {
  ...
  async fetchAppointments(_since?: Date): Promise<SyncedAppointmentData[]> {
    throw new NotImplementedError('emekaniker', 'fetchAppointments');
  }

  async handleWebhook(_payload: unknown, _signature: string): Promise<void> {
    throw new NotImplementedError('emekaniker', 'handleWebhook');
  }
}
