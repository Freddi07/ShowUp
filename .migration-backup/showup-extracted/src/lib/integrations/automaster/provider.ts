...
export class AutomasterProvider implements IntegrationProvider {
  ...
  async fetchAppointments(_since?: Date): Promise<SyncedAppointmentData[]> {
    throw new NotImplementedError('automaster', 'fetchAppointments');
  }

  async handleWebhook(_payload: unknown, _signature: string): Promise<void> {
    throw new NotImplementedError('automaster', 'handleWebhook');
  }
}
